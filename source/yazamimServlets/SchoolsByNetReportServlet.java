...
public class SchoolsByNetReportServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		if (areaNum != null && areaNum != "-1")
		{
			...
			areaName = DBQueries.getAreaByNum(area)
				.getAreaName();
		}else
			areaName = "כל האזורים";
		...
		if (area == null || area == 0) {
			if (netId == null || netId == 0) {
				queryText = DBQueries.reportSchoolsByNet()
						+ " ORDER BY netName";

			} else {
				queryText = DBQueries.reportSchoolsByNet() + " WHERE sn.netId="
						+ netId + " ORDER BY netName";

			}
		} else {
			if (netId == null || netId == 0) {
				queryText = DBQueries.reportSchoolsByNet()
						+ " WHERE c.areaNum =" + areaNum + " ORDER BY netName";

			} else {
				queryText = DBQueries.reportSchoolsByNet()
						+ " WHERE c.areaNum =" + areaNum + " and sn.netId="
						+ netId + " ORDER BY netName";
		...
		cell9.setCellValue("סוג בית ספר   ");
		HSSFCell cell10 = rowhead.createCell(10);
		cell10.setCellStyle(cellStyle);
		cell10.setCellValue("רשת בית ספר   ");
		int index = 1;
		try {
			while (rs.next()) {
				...
				row.createCell(5)
						.setCellValue(rs.getString("contactName"));
				row.createCell(6).setCellValue(
						rs.getString("contactPhone"));
				row.createCell(7)
						.setCellValue(rs.getString("fax"));
		...
		String filename = "schoolsByNet.xls";
